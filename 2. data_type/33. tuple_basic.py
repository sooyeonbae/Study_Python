...
- 튜플은 값의 집합이라는 측면에서는 리스트와 유사하지만
    값을 한 번 저장한 후에는 내부요소의 편집이 불가능합니다. (고정리스트)
- 튜플은 상수리스트 라고도 부르며 ()를 사용하여 표현합니다.
...
tu = 1, 3, 5, 7, 9
print(type(tu)) # : <class 'tuble'>


# 튜플은 리스트와 마찬가지로 unpackiging이 가능합니다. (변수의 개수와 리스트 요소 개수가 일치하면 하나씩 들어간다.)
n1, n2, n3, n4, n5 = tu
print(n1, n2, n3, n4, n5)



''' 튜플로 가능한 문법 (내부요소값을 바꾸지 않는 행위라면 가능)'''
# 값 가져오기
print(tu[2])
...
# 튜플+튜플 -> 튜플이 연결됨
print(tu + (10,11)) # : (1, 3, 5, 7, 9, 10, 11)

# 튜플 곱하기
print(tu * 2) # : (1, 3, 5, 7, 9, 1, 3, 5, 7, 9)



''' 튜플로 불가능한 문법 (내부 요소값을 바꾸는 행위. 원본을 해치는 행위) '''
# tu[2] = 11  인덱스로 접근해서 새로운 값 집어넣는것
# tu.append(10) 튜플에 새 값 넣기
# del(tu[2]) 인덱스로 값 지우기
