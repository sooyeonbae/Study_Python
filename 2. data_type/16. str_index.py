...
- 문자열은 메모리상에 낱말(단일문자) 하나하나가 일렬로 늘어선 형태로 저장되는 시퀀스 구조입니다.
...
print(s)
print(s[2]) # 자바에선 메서드.charAt()로 접근해야했지만 파이썬에선 이렇게 가능
print(s[-1]) # = 오른쪽에서부터 1번째 (= 길이-1 = '5'번 인덱스)
print(s[-3]) # =(길이:)6 -3 = '3'번 인덱스

# 인덱스 범위를 초과한 값을 참조하면 에러가 발생합니다.

# 문자열도 리스트처럼 순차적 자료형이기 때문에
# 반복문 for를 통해 단일문자 반복이 가능합니다.
for n in s :
    print(n, end="!")
