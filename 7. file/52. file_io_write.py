...
try :
    # 파일저장기능 (write, append)
    # 파일 경로
    file_path = r'/Users/baesy/Desktop/web_development/Python/test/test.txt'     # r : 백슬래시(여기엔없지만)가 탈출코드로 인식될까봐
                                                                    # 파일명 : test.txt

    # 파일 입출력을 실행하는 내장 함수 open()
    # 반환값으로 파일 입출력을 담당하는 객체가 리턴됩니다.
    # 첫번째 인수로 파일 경로를, 두번재 인수로 모드를 설정하시면 됩니다.
    # f = open(file_path, 'w')
    ...
    # 덮어쓰기 - 기존 경로가 없는 경우
    file_path = r'/Users/baesy/Desktop/web_development/Python/test/test2.txt' 
    f = open(file_path, 'a')
    text = 'a모드는 기존 파일만 지정할 수 있는 것이 아닌 새 파일'
